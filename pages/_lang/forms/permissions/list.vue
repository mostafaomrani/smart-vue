<template>
  <div id="pageTable">
    <v-container grid-list-xl fluid>
      <v-layout row wrap>
        <v-flex sm12>
          <h3>{{$t('permission.pagetitle')}}</h3>
        </v-flex>

        <v-flex sm12 class="text-left">
          <v-btn color="primary" dark :to="'/' + $i18n.locale +  '/forms/permissions/create'" target="_self'">
...
export default {
  ...
  methods: {
    ...
    change(id) {
      var self = this;
      getData("get", "/api/Permissions/" + id + "/permissions", {}, {}).then((res) => {
