<template>
  <div id="pageTable">
    <v-container grid-list-xl fluid>
      <v-layout row wrap>
        <v-flex sm12>
          <h3>{{$t('client.pagetitle')}}</h3>
          <div>- Clients can access to resorces behalf of end users.
          There are three types of clients : <strong>Server-side</strong>, <strong>Client-side</strong> (Angular, Vue, React ...) and <strong>Mobile</strong> (Android and Ios)</div>
        </v-flex>

         <v-flex sm12 class="text-left">         

          <v-btn color="primary" dark :to="'/' + $i18n.locale +  '/forms/clients/create'" target="_self">
...
export default {
  ...
  methods: {
    ...
    change(id) {
      var self = this;
      getData("get","/api/Clients/" + id  + "/Clients", {},{}).then(
        (res) => {
